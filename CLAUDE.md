...
- **Rust 64KB-constant limit (forward-compat envelope work, 2026-09):** `baboon_codecs_facade.rs` crossed the same JVM string-constant limit, so `BaboonTypeMeta` and `mod baboon_type_meta_codec` live in `baboon_type_meta.rs` and are re-exported from the facade (all `crate::baboon_codecs_facade::BaboonTypeMeta` paths still resolve). The compile error to recognise is `UTF8 string too large ... bad String constant of length N` while emitting `BaboonRuntimeResources$`; the fix is always to split the runtime file and add an `Output` entry for the new file in the translator's shared-runtime list.
