      ...
      for {
        ...
        _ <- F
          .ifThenElse(unexpectedFiles.isEmpty)(
            F.fromAttempt(targetPaths.foreach(path => IzFiles.erase(path.toPath)))
              .catchAll(t => F.fail(BaboonIssue.of(IOIssue.CantCleanupTarget(unexpectedFiles.map(_.toString), targetOptions.safeToRemoveExtensions.toSeq, Some(t))))),
      ...
      F.traverse_(options.metaWriteEvolutionJsonTo) {
        maybePath =>
          val path = Option(maybePath.getParent) match {
            case Some(_) => maybePath.toPath
            case None    => target.output.output.resolve(maybePath.asString)
          }
