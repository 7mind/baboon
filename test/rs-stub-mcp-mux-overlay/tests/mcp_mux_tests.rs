...
///
/// The SAME test source covers BOTH the sync and the async lanes: in async
/// mode (`--rs-async-services=true`) the generated `route_tool_call` drives its
...
/// and this test — are identical in both modes.
///
...
impl UserService for StubUserService {
    fn create_user(&self, _arg: CreateUserIn) -> CreateUserOut {
        CreateUserOut { profile: profile() }
    }
    fn get_user(&self, _arg: GetUserIn) -> GetUserOut {
...
impl OrderService for StubOrderService {
    fn place_order(&self, _arg: PlaceOrderIn) -> PlaceOrderOut {
        PlaceOrderOut { summary: OrderSummary { order_id: "o1".to_string(), status: OrderStatus::Confirmed, total: 10.0 } }
    }
    fn cancel_order(&self, _arg: CancelOrderIn) -> CancelOrderOut {
...
// ---------------------------------------------------------------------------
// Delegate factories: the SAME errors-mode invoke the integrator would bind.
// In async mode the generated MCP server adapts these via block_on internally.
// ---------------------------------------------------------------------------
...
fn user_invoke() -> McpJsonInvoke<()> {
    Box::new(|method: &BaboonMethodId, data: &str, _ctx: (), cc: &BaboonCodecContext| {
        ...
        invoke_json_user_service(method, data, &StubUserService, &rt, cc)
    })
}
...
fn order_invoke() -> McpJsonInvoke<()> {
    Box::new(|method: &BaboonMethodId, data: &str, _ctx: (), cc: &BaboonCodecContext| {
        let rt = BaboonServiceRtDefault;
        invoke_json_order_service(method, data, &StubOrderService, &rt, cc)
    })
}
