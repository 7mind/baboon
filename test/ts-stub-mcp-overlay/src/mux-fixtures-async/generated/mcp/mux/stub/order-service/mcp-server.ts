import { AbstractAsyncBaboonMcpServer, McpServerInfo, McpToolEntry, BaboonEitherResult } from '../../../../BaboonMcpRuntime';
...
// Transport-abstract: `handle` is inherited from AbstractAsyncBaboonMcpServer and
...
export class OrderServiceMcpServer<Ctx> extends AbstractAsyncBaboonMcpServer<Ctx> {
    ...
    private readonly _invokeJson: (method: BaboonMethodId, data: string, ctx: Ctx, codecCtx: BaboonCodecContext) => Promise<BaboonEitherResult>;

    constructor(invokeJson: (method: BaboonMethodId, data: string, ctx: Ctx, codecCtx: BaboonCodecContext) => Promise<BaboonEitherResult>) {
        super();
        this._invokeJson = invokeJson;
    }

    protected async invokeJson(method: BaboonMethodId, data: string, ctx: Ctx, codecCtx: BaboonCodecContext): Promise<BaboonEitherResult> {
