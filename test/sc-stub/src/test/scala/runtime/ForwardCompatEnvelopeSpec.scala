...
class ForwardCompatEnvelopeSpec extends AnyFunSuite {
  ...
  test("writer publishes $rv only where the json-additive bound is older than the identical bound") {
    val appended = writer.encodeToJson(fwde2e.fwd.FwdAppendVar(42, "hello", Some("appended"))).toTry.get
    ...
    val inserted = writer.encodeToJson(fwde2e.fwd.FwdMidInsert(7, Some(99), "zed")).toTry.get
    ...
    val stable = writer.encodeToJson(fwde2e.fwd.FwdStable("same")).toTry.get
    ...
    val enumHost = writer.encodeToJson(fwde2e.fwd.FwdEnumHost(fwde2e.fwd.FwdEnumGrows.C)).toTry.get
  ...
  test("envelope round-trips $rv through readMeta") {
    val appended = writer.encodeToJson(fwde2e.fwd.FwdAppendVar(1, "b", Some("t"))).toTry.get
