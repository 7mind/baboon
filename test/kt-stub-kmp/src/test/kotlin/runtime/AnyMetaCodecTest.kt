...
// only by the kt-stub-kmp codegen path (rsync + codegen into target/test-regular/kt-stub-kmp/).
...
//
// Mirrored from test/kt-stub/src/test/kotlin/runtime/AnyMetaCodecTest.kt for PR-25.5a
// (M25 KMP runtime test parity). KMP-specific adaptations:
//   - LEDataOutputStream/LEDataInputStream → BaboonBinaryWriter/BaboonBinaryReader
//     (KMP runtime uses the buffer types directly; no java.io.* streams).
//   - writer.flush() removed (BaboonBinaryWriter is a self-buffering byte array).
//   - baos.toByteArray() → writer.toByteArray().
//   - LEDataInputStream(ByteArrayInputStream(bytes)) → BaboonBinaryReader(bytes).
package runtime
...
class AnyMetaCodecTest {
    ...
    fun anyMetaCodec_writeBin_readBin_roundTripsAcrossAllSixKinds() {
        for ((kind, domain, version, typeid) in cases) {
            ...
            val out = BaboonBinaryWriter()
            AnyMetaCodec.writeBin(meta, out)

            val reader = BaboonBinaryReader(out.toByteArray())
    ...
    fun anyMetaCodec_writeBin_kind0x07_emits7Bytes() {
        ...
        val out = BaboonBinaryWriter()
        AnyMetaCodec.writeBin(meta, out)
        assertEquals(7, out.toByteArray().size)
    }
