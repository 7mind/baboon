// PR-57b KMP-Kotlin per-stub identifier-repr tests.
//
// Although the stub project itself targets `kotlin("jvm")` for test execution,
// the generated code is multiplatform (`--kt-multiplatform=true`), so it uses
// kotlinx.datetime.Instant, BaboonOffsetDateTime, and kotlin.uuid.Uuid instead
// of java.time.OffsetDateTime / java.util.UUID. Tests therefore reference KMP
// types only, mirroring what production multiplatform consumers would write.
//
// Runs only from the rsync'd codegen copy under target/test-regular/kt-stub-kmp/.
...
class IdentifierReprTest {
    ...
    fun tsuToString_24Chars() {
        // kotlinx.datetime.Instant.parse accepts the 24-char form natively.
        val instant = Instant.parse("2026-04-29T12:34:56.789Z")
        val s = BaboonIdentifierRepr.tsuToString(instant)
    ...
    fun tsoToString_29Chars_NeverZ() {
        val dt = BaboonOffsetDateTime(
            instant = Instant.parse("2026-04-29T10:34:56.789Z"),
            offset  = UtcOffset(seconds = 2 * 3600),
        )
        val s = BaboonIdentifierRepr.tsoToString(dt)
        assertEquals(29, s.length)
        assertEquals("2026-04-29T12:34:56.789+02:00", s)

        val utc = BaboonOffsetDateTime(
            instant = Instant.parse("2026-04-29T12:34:56.789Z"),
    ...
    fun mixed_Roundtrip_EmptyBytes_AndUtcTimes() {
        val src = Mixed(
            true,
            Uuid.parse("de7b9e1e-5c93-45fe-beec-da99994f629a"),
            ByteString.of(ByteArray(0)),
            Instant.parse("2026-04-29T12:34:56.789Z"),
            BaboonOffsetDateTime(
                instant = Instant.parse("2026-04-29T10:34:56.789Z"),
                offset  = UtcOffset(seconds = 2 * 3600),
            )
        )
