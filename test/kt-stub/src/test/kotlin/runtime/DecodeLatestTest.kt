...
class DecodeLatestTest {
    ...
    fun decodeFromJsonLatest_string_roundTripsInner() {
        val facade = freshFacade()
        val original = my.ok.Inner(42)
        // kt facade's encodeToJson emits content-only (asymmetric with decodeFromJson which
        // expects an envelope); construct the {$d,$v,$t,$c} envelope manually for the test.
