...
final class McpTests: XCTestCase {
    ...
    private let refListCollections = ##"{"$schema":"https://json-schema.org/draft/2020-12/schema","type":"object","properties":{"tags":{"type":"array","items":{"type":"string"}},"uniqueIds":{"type":"array","items":{"type":"string","format":"int64"},"uniqueItems":true},"labels":{"type":"object","additionalProperties":{"type":"string"}},"byColor":{"type":"object","additionalProperties":{"type":"string"},"propertyNames":{"type":"string","enum":["Red","Green","Blue"]}}},"required":["tags","uniqueIds","labels","byColor"],"$defs":{"mcp_stub_Color":{"type":"string","enum":["Red","Green","Blue"]}}}"##
    ...
    private let refProcessShape = ##"{"$schema":"https://json-schema.org/draft/2020-12/schema","type":"object","properties":{"shape":{"$ref":"#/$defs/mcp_stub_Shape"},"tree":{"$ref":"#/$defs/mcp_stub_Tree"}},"required":["shape","tree"],"$defs":{"mcp_stub_Shape":{"oneOf":[{"$ref":"#/$defs/mcp_stub_Shape_Circle"},{"$ref":"#/$defs/mcp_stub_Shape_Rect"}],"description":"ADT discriminator-wrapper encoding: encoded as a single-key JSON object {\"<Branch>\": { ...branch fields... }} where the key is the branch's short name and the value is that branch's field object. Exactly one branch key is present. Known branches: Circle, Rect."},"mcp_stub_Tree":{"type":"object","properties":{"value":{"type":"integer","format":"int32"},"left":{"oneOf":[{"$ref":"#/$defs/mcp_stub_Tree"},{"type":"null"}]},"children":{"type":"array","items":{"$ref":"#/$defs/mcp_stub_Tree"}}},"required":["value","children"]},"mcp_stub_Shape_Circle":{"type":"object","properties":{"radius":{"type":"number","format":"double"}},"required":["radius"]},"mcp_stub_Shape_Rect":{"type":"object","properties":{"w":{"type":"number","format":"double"},"h":{"type":"number","format":"double"}},"required":["w","h"]}}}"##
    ...
    private let refProcessTagged = ##"{"$schema":"https://json-schema.org/draft/2020-12/schema","type":"object","properties":{"tagged":{"$ref":"#/$defs/mcp_stub_Tagged"}},"required":["tagged"],"$defs":{"mcp_stub_Tagged":{"oneOf":[{"$ref":"#/$defs/mcp_stub_Tagged_TagA"},{"$ref":"#/$defs/mcp_stub_Tagged_TagB"}],"description":"ADT discriminator-wrapper encoding: encoded as a single-key JSON object {\"<Branch>\": { ...branch fields... }} where the key is the branch's short name and the value is that branch's field object. Exactly one branch key is present. Known branches: TagA, TagB."},"mcp_stub_Tagged_TagA":{"type":"object","properties":{"id":{"type":"string"},"tag":{"type":"string"}},"required":["id","tag"]},"mcp_stub_Tagged_TagB":{"type":"object","properties":{"id":{"type":"string"},"weight":{"type":"integer","format":"int32"}},"required":["id","weight"]}}}"##
