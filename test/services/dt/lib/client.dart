...
Future<void> runClient(String host, int port) async {
  ...
  try {
    ...
    // and unsuffixed method names; when UEBA is also active the client would
    // take a second transport and expose UEBA-default + `*Json` methods.)
    ...
    // Add Buddy
    final addBuddyOut = await petStore.addPet(
    ...
    // Add Whiskers
    final addWhiskersOut = await petStore.addPet(
    ...
    // List pets (expect 2)
    final listOut = await petStore.listPets(listpets.in_());
    assert(listOut.pets.length == 2, 'expected 2 pets, got ${listOut.pets.length}');

    // Get Buddy
    final getBuddyOut = await petStore.getPet(getpet.in_(id: buddyId));
    ...
    // Delete Whiskers
    final deleteOut = await petStore.deletePet(deletepet.in_(id: whiskersId));
    assert(deleteOut.deleted == true, 'expected deleted=true, got ${deleteOut.deleted}');

    // List pets again (expect 1)
    final list2Out = await petStore.listPets(listpets.in_());
