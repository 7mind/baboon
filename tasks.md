...
- [~] **PR-29P.3** — Close-out: live-CI verification on `wip/ids-and-adts` (both jobs green); add deferred-policy comment to workflow per `[PR-29P.2-D05]`; tick `[PR-29P.2-D06]` cross-cutting policy item; record M29 follow-ups (D02 non-UTC tso test coverage, D03 RandomJsonGenerator parity); write session log.
...
- **`RandomJsonGenerator` tso/tsu formatter split** — apply the same split as `BaboonRuntimeCodec` (`tsoFormatter` `+00:00` literal, `tsuFormatter` `Z` literal) in `baboon-compiler/src/main/scala/io/septimalmind/baboon/explore/RandomJsonGenerator.scala:14-17, 102-111`. User-facing REPL output only; no wire-format impact, but currently violates M28-N01 in `:example` output. Tracked as `[PR-29P.1-D03]`.
- **Non-UTC tso offset regression coverage** — extend `RTCodecTest` regression to cover non-UTC `tso` offsets (e.g. `+05:30`, `-08:00`) so a future change that hard-codes `+00:00` would surface immediately. Tracked as `[PR-29P.1-D02]`.
