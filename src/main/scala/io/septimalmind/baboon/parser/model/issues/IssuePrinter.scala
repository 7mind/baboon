...
object IssuePrinter {
  ...
  implicit val nonUniqueDomainVersionsPrinter
    : IssuePrinter[NonUniqueDomainVersions] =
  ...
  implicit val nonUniqueEnumBranchesPrinter
    : IssuePrinter[NonUniqueEnumBranches] = (issue: NonUniqueEnumBranches) => {
  ...
  implicit val unexpectedScopeLookupPrinter
    : IssuePrinter[UnexpectedScopeLookup] = (issue: UnexpectedScopeLookup) => {
    s"""${extractLocation(issue.meta)}
  ...
  implicit val nonUniqueDiffsPrinter: IssuePrinter[NonUniqueDiffs] =
    new BugPrinter[NonUniqueDiffs] {
      override def errorMessage(bug: NonUniqueDiffs): String = {
        val nonUniqueDiffsStr = bug.nonUniqueDiffs
          .map {
            case (typeId, diffs) =>
              s"""Type:        ${typeId.name}
  ...
  implicit val referentialCyclesFoundPrinter
    : IssuePrinter[ReferentialCyclesFound] =
  ...
  implicit val conflictingEnumBranchesPrinter
    : IssuePrinter[ConflictingEnumBranches] =
  ...
  implicit val conflictingAdtBranchesPrinter
    : IssuePrinter[ConflictingAdtBranches] =
  ...
  implicit val incorrectConversionApplicationPrinter
    : IssuePrinter[IncorrectConversionApplication] =
  ...
  implicit val setsCantContainGenericsPrinter
    : IssuePrinter[SetsCantContainGenerics] =
  ...
  implicit val mapKeysShouldNotBeGenericPrinter
    : IssuePrinter[MapKeysShouldNotBeGeneric] =
  ...
  implicit val eitherAllOrNoneEnumMembersMustHaveConstants
    : IssuePrinter[EitherAllOrNoneEnumMembersMustHaveConstants] = (issue) => {
    s"""Either all or none of enum members must have associated constants in ${issue.e.id.toString}""".stripMargin
  }
  implicit val underscoredDefinitionRetained
    : IssuePrinter[UnderscoredDefinitionRetained] = issue => {
