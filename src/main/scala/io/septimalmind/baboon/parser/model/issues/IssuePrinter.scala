...
object IssuePrinter {
  ...
  implicit val cantReadInputPrinter: IssuePrinter[CantReadInput] = (issue: CantReadInput) => {
    s"""Can't read form file: ${issue.path}
       |Due to: ${issue.throwable.stacktraceString}
       |""".stripMargin
  }

  implicit val cantWriteOutputPrinter: IssuePrinter[CantWriteOutput] = (issue: CantWriteOutput) => {
    s"""Can't write to file: $issue.path
       |Due to: ${issue.throwable.stacktraceString}
       |""".stripMargin
  }

  implicit val parserFailedPrinter: IssuePrinter[ParserFailed] = (issue: ParserFailed) => {
    val Array(line, character, _*) = issue.error.extra.input.prettyIndex(issue.error.index).split(":")
    s"Parser error occurred on => line:$line position:$character".stripMargin
  }

  implicit val scalaExpectedPrinter: IssuePrinter[ScalarExpected] = new BugPrinter[ScalarExpected] {
  ...
  implicit val collectionExpectedPrinter: IssuePrinter[CollectionExpected] = new BugPrinter[CollectionExpected] {
  ...
  implicit val nonUniqueDomainVersionsPrinter: IssuePrinter[NonUniqueDomainVersions] = (issue: NonUniqueDomainVersions) => {
    val stringProblems = issue.duplicateDomainVersion
      .map {
        case (version, domains) =>
          s"""Version: $version => models: ${domains.map(_.id.toString).mkString(", ")}
             |""".stripMargin
      }
      .mkString("\n")
  ...
  implicit val emptyDomainFamilyPrinter: IssuePrinter[EmptyDomainFamily] = new BugPrinter[EmptyDomainFamily] {
    override def errorMessage(bug: EmptyDomainFamily): String = {
      s"Empty model: ${bug.pkg.toString}"
    }
  }

  implicit val nonUniqueLineagesPrinter: IssuePrinter[NonUniqueLineages] = (issue: NonUniqueLineages) => {
    s"Non unique models found: ${issue.nonUniqueLineages.keys.toList.mkString(", ")}"
  }

  implicit val emptyFamilyPrinter: IssuePrinter[EmptyFamily] = new BugPrinter[EmptyFamily] {
    override def errorMessage(bug: EmptyFamily): String = {
      s"Empty models in files:${bug.input.niceList()}"
    }
  }

  implicit val unexpectedBuiltinPrinter: IssuePrinter[UnexpectedBuiltin] = (issue: UnexpectedBuiltin) => {
  ...
  implicit val unexpectedNonBuiltin: IssuePrinter[UnexpectedNonBuiltin] = (issue: UnexpectedNonBuiltin) => {
    s"""${extractLocation(issue.meta)}
       |Model: ${issue.pkg.toString}
       |The type ${issue.name.name} is not supported
       |Consider using one of our builtin types: ${
      TypeId.Builtins.all
        .map(_.name.name)
        .mkString(" ")
    }
  ...
  implicit val missingTypeIdPrinter: IssuePrinter[MissingTypeId] = (issue: MissingTypeId) => {
    s"""${extractLocation(issue.meta)}
  ...
  implicit val emptyEnumPrinter: IssuePrinter[EmptyEnum] = (issue: EmptyEnum) => {
    s"""${extractLocation(issue.meta)}
       |Found empty enum: ${issue.id.toString}
       |""".stripMargin
  }

  implicit val nonUniqueFieldsPrinter: IssuePrinter[NonUniqueFields] = (issue: NonUniqueFields) => {
    s"""${extractLocation(issue.meta)}
       |DTO: ${issue.id.toString} has fields with identical names:${issue.duplicateFields.values.flatten.niceList()}
       |""".stripMargin
  }
  ...
  implicit val emptyGenericArgsPrinter: IssuePrinter[EmptyGenericArgs] = (issue: EmptyGenericArgs) => {
    s"""${extractLocation(issue.meta)}
       |The type ${issue.id.name.name} is expected to be used with type argument
       |""".stripMargin
  }

  implicit val duplicatedTypedefsPrinter: IssuePrinter[DuplicatedTypedefs] = (issue: DuplicatedTypedefs) => {
    ...
    s"""${extractLocation(issue.model.header.meta)}
       |Duplicate type definitions have been found:${duplicateTypesString.niceList()}
       |""".stripMargin
  }

  implicit val circularDependencyPrinter: IssuePrinter[CircularDependency] = (issue: CircularDependency) => {
    val stringProblem = issue.error match {
      case ToposortError.UnexpectedLoop(_, matrix) =>
        matrix.links.map {
          case (typeId, dependencies) => s"type: ${typeId.name} => dependencies: ${dependencies.map(_.name.name).mkString(", ")}"
        }.niceList()
      case _ => ""
    }
    s"""${extractLocation(issue.model.header.meta)}
  ...
  implicit val emptyPackageIdPrinter: IssuePrinter[EmptyPackageId] = (issue: EmptyPackageId) => {
    s"""${extractLocation(issue.header.meta)}
       |There is an empty package name
       |""".stripMargin
  }

  implicit val nonUniqueTypedefsPrinter: IssuePrinter[NonUniqueTypedefs] = (issue: NonUniqueTypedefs) => {
    s"""${extractLocation(issue.meta)}
       |Duplicate type members found: ${issue.duplicateTypedefs.values.flatten.map(_.id.name.name).mkString}
       |""".stripMargin
  }

  implicit val nonUniqueScopePrinter: IssuePrinter[NonUniqueScope] = (issue: NonUniqueScope) => {
    s"""${extractLocation(issue.meta)}
       |Duplicate scopes have been found:${issue.nonUniqueScopes.map(_._2.map(_.name.name).mkString(", ")).niceList()}
       |""".stripMargin
  }

  implicit val unexpectedScopingPrinter: IssuePrinter[UnexpectedScoping] = new BugPrinter[UnexpectedScoping] {
  ...
  implicit val scopeCannotBeEmptyPrinter: IssuePrinter[ScopeCannotBeEmpty] = (issue: ScopeCannotBeEmpty) => {
  ...
  implicit val badEnumNamePrinter: IssuePrinter[BadEnumName] = (issue: BadEnumName) => {
    s"""${extractLocation(issue.meta)}
       |Bad enum name: $issue.name
       |""".stripMargin
  }

  implicit val badFieldNamePrinter: IssuePrinter[BadFieldName] = (issue: BadFieldName) => {
    s"""${extractLocation(issue.meta)}
       |Bad field name: $issue.name
       |""".stripMargin
  }

  implicit val badTypeNamePrinter: IssuePrinter[BadTypeName] = (issue: BadTypeName) => {
    s"""${extractLocation(issue.meta)}
       |Bad type name: ${issue.name}
       |""".stripMargin
  }

  implicit val badInheritancePrinter: IssuePrinter[BadInheritance] = new BugPrinter[BadInheritance] {
  ...
  implicit val circularInheritancePrinter: IssuePrinter[CircularInheritance] = (issue: CircularInheritance) => {
    val errorString = issue.error match {
      case ToposortError.UnexpectedLoop(_, matrix) =>
        matrix.links.map {
          case (typeId, children) => s"type: ${typeId.name.name} => children: ${children.toList.map(_.name.name).mkString(", ")}"
        }.niceList()
      case _ => ""
    }
    s"""${extractLocation(issue.meta)}
  ...
  implicit val nameNotFoundPrinter: IssuePrinter[NameNotFound] = (issue: NameNotFound) => {
    s"""${extractLocation(issue.meta)}
  ...
  implicit val namSeqeNotFoundPrinter: IssuePrinter[NamSeqeNotFound] = (issue: NamSeqeNotFound) => {
    s"""${extractLocation(issue.meta)}
       |Members: ${issue.names.map(_.name).mkString} are not found in scope: ${issue.scope.name.name}
       |""".stripMargin
  }

  implicit val duplicatedTypesPrinter: IssuePrinter[DuplicatedTypes] = (issue: DuplicatedTypes) => {
    s"""${extractLocation(issue.meta)}
       |Duplicate types have been found:${issue.dupes.map(_.name.name).niceList()}
       |""".stripMargin
  }

  implicit val wrongParentPrinter: IssuePrinter[WrongParent] = (issue: WrongParent) => {
    s"""${extractLocation(issue.meta)}
       |DTO parent is expected instead provided => name:${issue.id.toString} type:${issue.id1.name.name}
       |""".stripMargin
  }

  implicit val brokenComparisonPrinter: IssuePrinter[BrokenComparison] = new BugPrinter[BrokenComparison] {
  ...
  implicit val unexpectedDiffTypePrinter: IssuePrinter[UnexpectedDiffType] = new BugPrinter[UnexpectedDiffType] {
  ...
  implicit val nonUniqueDiffPrinter: IssuePrinter[NonUniqueDiff] = new BugPrinter[NonUniqueDiff] {
    override def errorMessage(bug: NonUniqueDiff): String = {
      val evolutionsStr = bug.evolutions
        .map {
  ...
  implicit val nonUniqueRulesetPrinter: IssuePrinter[NonUniqueRuleset] = new BugPrinter[NonUniqueRuleset] {
  ...
  implicit val incomparableTypedefsPrinter: IssuePrinter[IncomparableTypedefs] = (issue: IncomparableTypedefs) => {
  ...
  implicit val nonUniqueDiffsPrinter: IssuePrinter[NonUniqueDiffs] = new BugPrinter[NonUniqueDiffs] {
  ...
  implicit val mismatchingTypedefs: IssuePrinter[MismatchingTypedefs] = new BugPrinter[MismatchingTypedefs] {
  ...
  implicit val missingTypeDef: IssuePrinter[MissingTypeDef] = (issue: MissingTypeDef) => {
    s"""Model: ${issue.domain.id.toString}
       |Has missing type definitions: ${issue.missing.map(_.name.name).mkString}
       |""".stripMargin
  }

  implicit val referentialCyclesFoundPrinter: IssuePrinter[ReferentialCyclesFound] = (issue: ReferentialCyclesFound) => {
    val stringLoops = issue.loops.toList
      .map(cycle => s"type: ${cycle.node.name.name} => loop: ${cycle.loops.flatMap(_.loop.map(_.name.name)).mkString}")
      .niceList()
    s"""Model: ${issue.domain.id.toString}
  ...
  implicit val conflictingDtoFieldsPrinter: IssuePrinter[ConflictingDtoFields] = (issue: ConflictingDtoFields) => {
    s"""${extractLocation(issue.meta)}
       |In DTO: ${issue.dto.id.name.name}
       |Conflicting fields have been found:${issue.dupes.values.flatten.niceList()}
       |""".stripMargin
  }

  implicit val conflictingEnumBranchesPrinter: IssuePrinter[ConflictingEnumBranches] = (issue: ConflictingEnumBranches) => {
    s"""${extractLocation(issue.meta)}
       |In Enum: ${issue.uEnum.id.name.name}
       |Conflicting members have been found: ${issue.dupes.values.flatten.map(_.name).mkString(", ")}
       |""".stripMargin
  }

  implicit val conflictingAdtBranchesPrinter: IssuePrinter[ConflictingAdtBranches] = (issue: ConflictingAdtBranches) => {
    s"""${extractLocation(issue.meta)}
       |In ADT: ${issue.adt.id.name.name}
       |Conflicting branches have been found: ${issue.dupes.values.flatten.map(_.name.name).mkString(", ")}
       |""".stripMargin
  }

  implicit val conflictingTypeIdsPrinter: IssuePrinter[ConflictingTypeIds] = (issue: ConflictingTypeIds) => {
    s"""In model: ${issue.domain.id.toString}
       |Conflicting types have been found: ${issue.dupes.values.flatten.map(_.id.toString).mkString(", ")}
       |""".stripMargin
  }

  implicit val emptyEnumDefPrinter: IssuePrinter[EmptyEnumDef] = (issue: EmptyEnumDef) => {
    s"""${extractLocation(issue.meta)}
       |Empty enum: ${issue.e.id.toString}
       |""".stripMargin
  }

  implicit val emptyAdtDefPrinter: IssuePrinter[EmptyAdtDef] = (issue: EmptyAdtDef) => {
    s"""${extractLocation(issue.meta)}
       |Empty ADT: ${issue.a.id.toString}
       |""".stripMargin
  }

  implicit val missingEvoDiffPrinter: IssuePrinter[MissingEvoDiff] = new BugPrinter[MissingEvoDiff] {
  ...
  implicit val missingEvoConversionPrinter: IssuePrinter[MissingEvoConversion] = new BugPrinter[MissingEvoConversion] {
    override def errorMessage(bug: MissingEvoConversion): String = {
      s"""Missing evolution conversion
         |Missing conversions:${(bug.missingConversions ++ bug.extraConversions).niceList()}
         |In model:
  ...
  implicit val brokenConversionPrinter: IssuePrinter[BrokenConversion] = new BugPrinter[BrokenConversion] {
  ...
  implicit val incorrectConversionApplicationPrinter: IssuePrinter[IncorrectConversionApplication] = new BugPrinter[IncorrectConversionApplication] {
  ...
  implicit val pathologicGenericsPrinter: IssuePrinter[PathologicGenerics] = (issue: PathologicGenerics) => {
    s"""${extractLocation(issue.meta)}
  ...
  implicit val setsCantContainGenericsPrinter: IssuePrinter[SetsCantContainGenerics] = (issue: SetsCantContainGenerics) => {
    s"""${extractLocation(issue.meta)}
       |DTO: ${issue.dto.id.toString}
  ...
  implicit val mapKeysShouldNotBeGenericPrinter: IssuePrinter[MapKeysShouldNotBeGeneric] = (issue: MapKeysShouldNotBeGeneric) => {
    s"""${extractLocation(issue.meta)}
       |DTO: ${issue.dto.id.toString}
  ...
  implicit val nonUniqueOutputFilesPrinter: IssuePrinter[NonUniqueOutputFiles] = (issue: NonUniqueOutputFiles) => {
    s"""Non unique output files:${issue.c.niceList()}
       |""".stripMargin
  }

  implicit val incorrectRootFound: IssuePrinter[IncorrectRootFound] = (issue: IncorrectRootFound) => {
    s"""Incorrect root found in model: ${issue.domain.id.toString} ${issue.badRoots.map(_.name.name).niceList()}
       |""".stripMargin
  }
  ...
  implicit val typerIssue: IssuePrinter[TyperIssue] = {
    ...
    case i: NonUniqueDomainVersions => apply[NonUniqueDomainVersions].stringify(i)
  ...
  implicit val verificationIssuePrinter: IssuePrinter[VerificationIssue] = {
    ...
    case i: ConflictingEnumBranches => apply[ConflictingEnumBranches].stringify(i)
    ...
    case i: IncorrectConversionApplication => apply[IncorrectConversionApplication].stringify(i)
    case i: PathologicGenerics => apply[PathologicGenerics].stringify(i)
    case i: SetsCantContainGenerics => apply[SetsCantContainGenerics].stringify(i)
    case i: MapKeysShouldNotBeGeneric => apply[MapKeysShouldNotBeGeneric].stringify(i)
  ...
  implicit class IssuePrinterListOps[T <: BaboonIssue : IssuePrinter](issues: Seq[T]) {
