...
case class GenericTranspilerCLIOptions(
  ...
  @HelpMessage(
    "Most recent versions will not have version segment in its namespace"
  )
  omitMostRecentVersionSuffixFromNamespaces: Option[Boolean],
  @HelpMessage(
    "Path to a file to which the compiler will write evolution metadata as a JSON"
  )
...
case class CLIOptions(
  @HelpMessage(
    "A list of *.baboon files to process (can be combined with --model-dir)"
  )
  model: List[String],
