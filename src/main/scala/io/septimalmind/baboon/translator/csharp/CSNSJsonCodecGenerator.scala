            ...
            case u: DomainMember.User =>
              u.defn match {
                case _: Typedef.Enum | _: Typedef.Foreign =>
                  val targetTpe = trans.toCsTypeRefNoDeref(uid, domain, evo)
      ...
      case TypeRef.Constructor(id, args) =>
        id match {
          ...
          case TypeId.Builtins.map =>
            ...
            q"""$ref!.Value<$nsJObject>()!.Properties().Select(kv => new $csKeyValuePair<$keyType, $valueType>($keyDec, $valueDec)).ToImmutableDictionary()"""

          case TypeId.Builtins.lst =>
            q"""$ref!.Value<$nsJArray>()!.Select(e => ${mkDecoder(args.head, q"e")}).ToImmutableList()"""

          case TypeId.Builtins.set =>
            q"""$ref!.Value<$nsJArray>()!.Select(e => ${mkDecoder(args.head, q"e")}).ToImmutableHashSet()"""
