...
object BaboonTest {
  ...
  private implicit val quasiAsync: QuasiAsync[FThrowable[Either]#l] = {
    ...
    new QuasiAsync[FThrowable[Either]#l] {
      ...
      override def parTraverse[A, B](l: IterableOnce[A])(f: A => Either[Throwable, B]): Either[Throwable, List[B]]          = Right(id.parTraverse(l)(f(_).merge))
      override def parTraverse_[A](l: IterableOnce[A])(f: A => Either[Throwable, Unit]): Either[Throwable, Unit]            = Right(id.parTraverse_(l)(f(_).merge))
      override def parTraverseN[A, B](n: Int)(l: IterableOnce[A])(f: A => Either[Throwable, B]): Either[Throwable, List[B]] = Right(id.parTraverseN(n)(l)(f(_).merge))
      override def parTraverseN_[A](n: Int)(l: IterableOnce[A])(f: A => Either[Throwable, Unit]): Either[Throwable, Unit]   = Right(id.parTraverseN_(n)(l)(f(_).merge))
    }
  }
  ...
  private implicit val quasiIO: QuasiIO[FThrowable[Either]#l] = {
    ...
    new QuasiIO[FThrowable[Either]#l] {
      override def flatMap[A, B](fa: Either[Throwable, A])(f: A => Either[Throwable, B]): Either[Throwable, B] = id.flatMap(fa.merge)(f)
      override def guaranteeOnFailure[A](fa: => Either[Throwable, A])(cleanupOnFailure: Throwable => Either[Throwable, Unit]): Either[Throwable, A] =
        id.guaranteeOnFailure(fa)(cleanupOnFailure(_).merge)
      override def bracketCase[A, B](acquire: => Either[Throwable, A])(release: (A, Option[Throwable]) => Either[Throwable, Unit])(use: A => Either[Throwable, B])
        : Either[Throwable, B] = Right(id.bracketCase[A, B](acquire.merge)((a: A, o: Option[Throwable]) => release(a, o).merge)(use(_).merge))
      ...
      override def redeem[A, B](action: => Either[Throwable, A])(failure: Throwable => Either[Throwable, B], success: A => Either[Throwable, B]): Either[Throwable, B] =
        id.redeem(action.merge)(failure, success)
      ...
      override def tailRecM[A, B](a: A)(f: A => Either[Throwable, Either[A, B]]): Either[Throwable, B] = Right(id.tailRecM[A, B](a)(f(_).merge))
      override def bracket[A, B](acquire: => Either[Throwable, A])(release: A => Either[Throwable, Unit])(use: A => Either[Throwable, B]): Either[Throwable, B] =
        Right(id.bracket[A, B](acquire.merge)(release(_).merge)(use(_).merge))
      override def guarantee[A](fa: => Either[Throwable, A])(`finally`: => Either[Throwable, Unit]): Either[Throwable, A] = id.guarantee(fa)(`finally`.merge)
      override def traverse[A, B](l: Iterable[A])(f: A => Either[Throwable, B]): Either[Throwable, List[B]]               = Right(id.traverse(l)(f(_).merge))
      override def traverse_[A](l: Iterable[A])(f: A => Either[Throwable, Unit]): Either[Throwable, Unit]                 = Right(id.traverse_(l)(f(_).merge))
      ...
      override def map2[A, B, C](fa: Either[Throwable, A], fb: => Either[Throwable, B])(f: (A, B) => C): Either[Throwable, C] = Right(
        id.map2[A, B, C](fa.merge, fb.merge)(f)
      )
  ...
  private implicit val quasiIORunner: QuasiIORunner[FThrowable[Either]#l] = new QuasiIORunner[FThrowable[Either]#l] {
    override def run[A](f: => Either[Throwable, A]): A = f.merge
  }
