...
/** PR-30.10 emission test: verifies that `JvDefnTranslator` emits Javadoc-style
  ...
  * TypeScript, and Kotlin emission tests — the model content is
  * language-agnostic) and asserts the emitted Java source contains the
  * expected `/** … */` blocks per `docs/spec/docstrings.md` §7.2.
  *
  * Body text is HTML-escaped: `<` → `&lt;`, `>` → `&gt;`, `&` → `&amp;`.
  */
final class DocCommentJavaEmissionTest extends DocCommentJavaEmissionTestBase[Either]

abstract class DocCommentJavaEmissionTestBase[F[+_, +_]: Error2: TagKK: BaboonTestModule] extends BaboonTest[F] {
  ...
  private val jvTarget: JvTarget = JvTarget(
    ...
    output = OutputOptions(
      ...
      output                 = FSPath.parse(NEString.unsafeFrom("./target/baboon-jv-docs-test/")),
      fixturesOutput         = None,
      testsOutput            = None,
    ),
  ...
  "m30-sc-docs fixture, Java target" should {

    "emit /** type-level */ doc before record class (spec §7.2)" in {
    ...
    "emit /** type-level */ doc before enum (spec §7.2)" in {
      (loader: BaboonLoader[F], translator: BaboonAbstractTranslator[F]) =>
        for {
          family <- loadDocsFamily(loader)
          srcs   <- translator.translate(family)
    ...
    "emit /** type-level */ doc before sealed interface for ADT (spec §7.2)" in {
      (loader: BaboonLoader[F], translator: BaboonAbstractTranslator[F]) =>
        for {
          family <- loadDocsFamily(loader)
          srcs   <- translator.translate(family)
          ...
          val resultFile = all.collectFirst {
            case (_, c) if c.contains("sealed interface DocResult") => c
    ...
    "emit /** type-level */ and /** method-level */ docs in service interface (spec §7.2)" in {
      (loader: BaboonLoader[F], translator: BaboonAbstractTranslator[F]) =>
        for {
          family <- loadDocsFamily(loader)
          srcs   <- translator.translate(family)
    ...
    "emit /** arm-level */ doc before ADT branch record (PR-30.10 / spec §7.2)" in {
      (loader: BaboonLoader[F], translator: BaboonAbstractTranslator[F]) =>
        for {
          family <- loadDocsFamily(loader)
          srcs   <- translator.translate(family)
          ...
          val resultFile = all.collectFirst {
            case (_, c) if c.contains("public record DocOk(") => c
          ...
          assert(
            ...
            s"Expected arm-level /** */ doc before DocOk record.\n$resultFile",
          )
          // Field-level doc inside DocOk arm
          assert(
            resultFile.contains("/** the carried payload */"),
            s"Expected field /** */ doc for DocOk.value component.\n$resultFile",
          )
        }
    }
      ...
      (loader: BaboonLoader[F], translator: BaboonAbstractTranslator[F]) =>
        for {
          ...
          val pageFile = all.collectFirst {
            case (_, c) if c.contains("public record DocPage(") => c
