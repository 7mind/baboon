...
/** PR-57c emission test: runs the actual `SwDefnTranslator` pipeline against
  * the `identifier-ok` fixture and asserts the emitted Swift source for the
  * `id`-typed Dtos matches spec §6 canonical patterns.
  */
final class IdentifierSwiftEmissionTest extends IdentifierSwiftEmissionTestBase[Either]

abstract class IdentifierSwiftEmissionTestBase[F[+_, +_]: Error2: TagKK: BaboonTestModule] extends BaboonTest[F] {
  ...
  private val swTarget: SwTarget = SwTarget(
    ...
    output = OutputOptions(
      ...
      output                 = FSPath.parse(NEString.unsafeFrom("./target/baboon-scalatests-sw/")),
      fixturesOutput         = None,
      testsOutput            = None,
    ),
  ...
  "identifier-ok fixture, Swift target" should {

    "emit description (CustomStringConvertible) and parseRepr that match spec §6 canonical patterns" in {
      (loader: BaboonLoader[F], translator: BaboonAbstractTranslator[F]) =>
        for {
          ...
          val pointIdRendering = all.collectFirst {
            case (_, content) if content.contains("public struct PointId") => content
          }.getOrElse(fail(s"PointId struct not found. Paths: ${all.map(_._1)}"))
          ...
          // Q-FU-4: extension-based CustomStringConvertible (NOT struct method).
          assert(
            pointIdRendering.contains("extension PointId: CustomStringConvertible"),
            s"Expected `extension PointId: CustomStringConvertible`. Source:\n$pointIdRendering",
          )
          ...
          // Spec §5: parser entry point on PointIdCodec enum (Q-FU-4: not on struct).
          assert(
            pointIdRendering.contains("public enum PointIdCodec"),
            s"Expected `public enum PointIdCodec`. Source:\n$pointIdRendering",
          )
          assert(
            pointIdRendering.contains("public static func parseRepr"),
            s"Expected `public static func parseRepr`. Source:\n$pointIdRendering",
          )

          // Spec §3 / §5.4 fixed-width tsu/tso consumption.
          val mixedRendering = all.collectFirst {
            case (_, content) if content.contains("public struct Mixed") => content
          }.getOrElse(fail(s"Mixed struct not found."))
          assert(
          ...
          val uintsRendering = all.collectFirst {
            case (_, content) if content.contains("public struct UInts") => content
          }.getOrElse(fail("UInts not found"))
          assert(
          ...
          val markerRendering = all.collectFirst {
            case (_, content) if content.contains("public struct Marker") => content
          }.getOrElse(fail("Marker not found"))
          assert(
          ...
          val outerRendering = all.collectFirst {
            case (_, content) if content.contains("public struct Outer") => content
          }.getOrElse(fail("Outer not found"))
          assert(
            outerRendering.contains("PointIdCodec.parseReprCursor(cursor)"),
            s"Expected nested PointIdCodec.parseReprCursor dispatch. Source:\n$outerRendering",
          )
          assert(
            outerRendering.contains("\"{\" + self.ref.description + \"}\""),
            s"Expected `{...}` nested-id wrapper. Source:\n$outerRendering",
          )
