  ...
  "explorer context" should {
    "find regular types and aliases in v1" in {
      (loader: BaboonLoader[F], codec: BaboonRuntimeCodec[F]) =>
        withExploreCtx(loader, codec, v1) {
          ctx =>
            // Regular types
    ...
    "list v3 aliases" in {
      (loader: BaboonLoader[F], codec: BaboonRuntimeCodec[F]) =>
        withExploreCtx(loader, codec, v3) {
          ctx =>
            val names = ctx.allAliases.map(_.name.name).toSet
      ...
      (loader: BaboonLoader[F], codec: BaboonRuntimeCodec[F]) =>
        withExploreCtx(loader, codec, v3) {
          ctx =>
            ...
            dom.defs.meta.nodes.values.collect { case u: DomainMember.User => u }.foreach {
              member =>
