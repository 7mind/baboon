...
/** T16 codegen-shape test for the Dart MCP server generator.
  *
  * Drives the real `translate` path with `--dt-generate-mcp-server=true`, so the
  * T6 dispatch seam invokes [[io.septimalmind.baboon.translator.dart.DtMcpServerGenerator]],
  * and asserts the emitted Dart MCP server shape for the locked K6 stub model
  ...
  * inputSchema wired from the T5 emitter (embedded as `jsonDecode` literals).
  *
  * Mirrors [[KotlinMcpServerEmissionTest]] (T14).
  */
final class DartMcpServerEmissionTest extends DartMcpServerEmissionTestBase[Either]

abstract class DartMcpServerEmissionTestBase[F[+_, +_]: Error2: TagKK: BaboonTestModule] extends BaboonTest[F] {

  private def dtTarget(mcp: Boolean): DtTarget = DtTarget(
    id = "Dart",
    output = OutputOptions(
      safeToRemoveExtensions = Set.empty,
      runtime                = RuntimeGenOpt.With,
      generateConversions    = true,
      output                 = FSPath.parse(NEString.unsafeFrom("./target/baboon-scalatests-dt-mcp/")),
      fixturesOutput         = None,
      testsOutput            = None,
    ),
  ...
  private def moduleFor(target: DtTarget): distage.Module = {
    val baseModule =
    ...
    baseModule overriddenBy new BaboonJvmDtModule[Either](target)
  }

  override protected def config: TestConfig = super.config.copy(
    pluginConfig = PluginConfig.const(moduleFor(dtTarget(mcp = true)).morph[PluginBase]),
  ...
  "Dart MCP server generator (mcp-stub-ok fixture)" should {

    "emit a per-service MCP server + runtime with the contract shape when --dt-generate-mcp-server=true" in {
      (loader: BaboonLoader[F], translator: BaboonAbstractTranslator[F]) =>
        for {
          ...
          assert(
            files.keys.exists(_.endsWith("baboon_mcp_runtime.dart")),
            s"baboon_mcp_runtime.dart not emitted; files=${files.keys}",
          )

          // Exactly one per-service MCP server file (single service McpTools).
          val serverPaths = files.keys.filter(_.endsWith("mcp_tools_mcp_server.dart")).toList
          ...
          assert(
            server.contains("class McpToolsMcpServer<Ctx> extends AbstractBaboonMcpServer<Ctx>"),
            s"missing generic server class extending AbstractBaboonMcpServer:\n$server",
          )
          // It has an injected invokeJson delegate — NO I/O loop.
          assert(
            server.contains("String Function(BaboonMethodId, String, Ctx, BaboonCodecContext)"),
            s"missing McpJsonInvoke delegate signature:\n$server",
          )
          assert(
            server.contains("invokeJsonFn"),
            s"missing invokeJsonFn getter:\n$server",
          )
          ...
          // NOTE: In Dart single-quoted strings, `$` is escaped as `\$`.
          assert(
          ...
          // Color enum schema has "enum" key.
          assert(
            server.contains(""""enum":"""),
            s"Color enum schema (T5 emitter) missing:\n$server",
          )
          // Schema is carried as a jsonDecode literal.
          assert(
            server.contains("jsonDecode("),
            s"inputSchema must be embedded as a jsonDecode literal:\n$server",
          )
          // The package import for the runtime is present.
          assert(
            server.contains("package:baboon_runtime/baboon_mcp_runtime.dart"),
            s"baboon_mcp_runtime import missing:\n$server",
          )
        }
    }
  }
