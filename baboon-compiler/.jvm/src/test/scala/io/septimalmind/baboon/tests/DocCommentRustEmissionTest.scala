...
/** PR-30.7 emission test: verifies that `RsDefnTranslator` emits Rust outer-line
  * `///` doc comments for doc-bearing types, fields, and service methods.
  *
  * Uses the `m30-sc-docs` fixture (shared with the Scala, C#, and Python emission
  * tests — the model content is language-agnostic) and asserts the emitted Rust
  * source contains the expected `///` lines per `docs/spec/docstrings.md` §7.7.
  *
  * Per spec §7.7: `//!` (module-inner form) must NOT appear in generated output.
  */
final class DocCommentRustEmissionTest extends DocCommentRustEmissionTestBase[Either]

abstract class DocCommentRustEmissionTestBase[F[+_, +_]: Error2: TagKK: BaboonTestModule] extends BaboonTest[F] {
  ...
  private val rsTarget: RsTarget = RsTarget(
    ...
    output = OutputOptions(
      ...
      output                 = FSPath.parse(NEString.unsafeFrom("./target/baboon-rs-docs-test/")),
      fixturesOutput         = None,
      testsOutput            = None,
    ),
  ...
  "m30-sc-docs fixture, Rust target" should {

    "emit /// type-level doc before pub struct (spec §7.7)" in {
      (loader: BaboonLoader[F], translator: BaboonAbstractTranslator[F]) =>
        for {
          ...
          val itemFile = all.collectFirst {
            case (_, c) if c.contains("pub struct DocItem") => c
          }.getOrElse(fail(s"DocItem not found. Paths: ${all.map(_._1)}"))

          // Type-level doc before `pub struct DocItem`
          assert(
          ...
          // Spec §7.7: //! must NOT appear in generated Rust output
          assert(
            !itemFile.contains("//!"),
            s"Found forbidden //! in generated Rust output for DocItem.\n$itemFile",
          )
        }
    }

    "emit /// type-level doc before pub enum for enum type (spec §7.7)" in {
      (loader: BaboonLoader[F], translator: BaboonAbstractTranslator[F]) =>
        for {
          family <- loadDocsFamily(loader)
          srcs   <- translator.translate(family)
          ...
          val severityFile = all.collectFirst {
            case (_, c) if c.contains("pub enum DocSeverity") => c
    ...
    "emit /// type-level doc before ADT pub enum (spec §7.7)" in {
      (loader: BaboonLoader[F], translator: BaboonAbstractTranslator[F]) =>
        for {
          family <- loadDocsFamily(loader)
          srcs   <- translator.translate(family)
          ...
          val resultFile = all.collectFirst {
            case (_, c) if c.contains("pub enum DocResult") => c
          }.getOrElse(fail(s"DocResult ADT not found. Paths: ${all.map(_._1)}"))

          assert(
            resultFile.contains("/// Result or error."),
            s"Expected type-level /// doc before DocResult enum.\n$resultFile",
          )
        }
    }

    "emit /// type-level and method-level docs in service trait (spec §7.7)" in {
      (loader: BaboonLoader[F], translator: BaboonAbstractTranslator[F]) =>
        for {
          family <- loadDocsFamily(loader)
          srcs   <- translator.translate(family)
          ...
          assert(
            ...
            s"Expected type-level /// doc before DocCrud trait.\n$crudFile",
          )
          assert(
    ...
    "emit /// arm-level doc before ADT branch struct (PR-30.7 / spec §7.7)" in {
      (loader: BaboonLoader[F], translator: BaboonAbstractTranslator[F]) =>
        for {
          family <- loadDocsFamily(loader)
          srcs   <- translator.translate(family)
          ...
          val resultFile = all.collectFirst {
            case (_, c) if c.contains("pub struct DocOk") => c
      ...
      (loader: BaboonLoader[F], translator: BaboonAbstractTranslator[F]) =>
        for {
          ...
          val pageFile = all.collectFirst {
            case (_, c) if c.contains("pub struct DocPage") => c
          ...
          assert(
            !pageFile.contains("/// \npub items"),
            s"Unexpected empty /// doc line before items field in DocPage.\n$pageFile",
          )
        }
    }
  }
