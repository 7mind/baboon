...
class BaboonModuleLogicModule[F[+_, +_]: Error2: MaybeSuspend2: TagKK](
  ...
  make[AdtDeltaMaterializer[F]]
  make[TemplateRegistryBuilder[F]].from[TemplateRegistryBuilder.Impl[F]]
...
class BaboonCommonScModule[F[+_, +_]: Error2: TagKK] extends ModuleDef {
  ...
  // The shared MCP inputSchema emitter (T5) reuses the OpenAPI scalar/any
  // fragment logic via OasTypeTranslator; bind it here so the Scala MCP generator
  // (T12) can construct the emitter. OasTypeTranslator is stateless.
  make[OasTypeTranslator]
  make[McpServerGeneratorHook[F]].from[ScMcpServerGenerator[F]]
...
class BaboonCommonRsModule[F[+_, +_]: Error2: TagKK] extends ModuleDef {
  ...
  // The shared MCP inputSchema emitter (T5) reuses the OpenAPI scalar/any
  // fragment logic via OasTypeTranslator; bind it here so the Rust MCP generator
  // (T13) can construct the emitter. OasTypeTranslator is stateless.
  make[OasTypeTranslator]
  make[McpServerGeneratorHook[F]].from[RsMcpServerGenerator[F]]
...
class BaboonCommonKtModule[F[+_, +_]: Error2: TagKK] extends ModuleDef {
  ...
  // The shared MCP inputSchema emitter (T5) reuses the OpenAPI scalar/any
  // fragment logic via OasTypeTranslator; bind it here so the Kotlin MCP generator
  // (T14) can construct the emitter. OasTypeTranslator is stateless.
  make[OasTypeTranslator]
  make[McpServerGeneratorHook[F]].from[KtMcpServerGenerator[F]]
...
class BaboonCommonJvModule[F[+_, +_]: Error2: TagKK] extends ModuleDef {
  ...
  // The shared MCP inputSchema emitter (T5) reuses the OpenAPI scalar/any
  // fragment logic via OasTypeTranslator; bind it here so the Java MCP generator
  // (T15) can construct the emitter. OasTypeTranslator is stateless.
  make[OasTypeTranslator]
  make[McpServerGeneratorHook[F]].from[JvMcpServerGenerator[F]]
