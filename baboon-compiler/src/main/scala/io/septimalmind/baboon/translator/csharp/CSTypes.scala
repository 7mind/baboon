...
object CSTypes {
  ...
  val iBaboonJsonService: CSType    = CSType(baboonRuntimePkg, "IBaboonJsonService", fq = false, CSTypeOrigin.Other)
  val iBaboonUebaService: CSType    = CSType(baboonRuntimePkg, "IBaboonUebaService", fq = false, CSTypeOrigin.Other)
  val iBaboonJsonServiceCtx: CSType = CSType(baboonRuntimePkg, "IBaboonJsonServiceCtx", fq = false, CSTypeOrigin.Other)
  val iBaboonUebaServiceCtx: CSType = CSType(baboonRuntimePkg, "IBaboonUebaServiceCtx", fq = false, CSTypeOrigin.Other)
  ...
  val baboonAnyJsonCodec: CSType   = CSType(baboonRuntimePkg, "BaboonAnyJsonCodec", fq = false, CSTypeOrigin.Other)
  val baboonAnyBinCodec: CSType    = CSType(baboonRuntimePkg, "BaboonAnyBinCodec", fq = false, CSTypeOrigin.Other)
  ...
  /** The full set of C# reserved keywords. These cannot be used bare as identifiers and require a
    * verbatim-identifier `@` prefix. Source: C# language specification §6.4.4 (Keywords).
    */
  val csReservedKeywords: Set[String] = Set(
    "abstract",
    "as",
    "base",
    "bool",
  ...
  /** Escape a C# identifier with a verbatim-identifier `@` prefix when it collides with a reserved
    * keyword. The `@` prefix is source-only and does not change the identifier's meaning or any wire
    * representation — `@default` and `default` denote the same identifier to the compiler, but only
    * the former is a legal identifier when the bare form is a keyword. Idempotent for non-keywords.
    */
  def escapeCsKeyword(identifier: String): String =
    if (csReservedKeywords.contains(identifier)) s"@$identifier" else identifier
}
