...
class SwConversionTranslator[F[+_, +_]: Error2](
  ...
  private def transfer(newTpe: TypeRef, oldRef: TextTree[SwValue], depth: Int, maybeOldTpe: Option[TypeRef] = None): TextTree[SwValue] = {
    ...
    (newTpe, oldTpe) match {
      ...
      case (_: TypeRef.Any, _) => AnyPlaceholder.notSupportedYet("SwConversionTranslator.transfer(new any)")
      case (_, _: TypeRef.Any) => AnyPlaceholder.notSupportedYet("SwConversionTranslator.transfer(old any)")
    }
  }
