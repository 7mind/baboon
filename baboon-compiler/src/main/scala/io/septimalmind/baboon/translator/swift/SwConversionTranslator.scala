...
class SwConversionTranslator[F[+_, +_]: Error2](
  ...
  private def transfer(newTpe: TypeRef, oldRef: TextTree[SwValue], depth: Int, maybeOldTpe: Option[TypeRef] = None): TextTree[SwValue] = {
    ...
    (newTpe, oldTpe) match {
      ...
      // `any` payload is opaque (`AnyOpaque`); we never auto-convert between `any` and a
      // non-`any` type (validator forbids it). When both old and new are `any` AND their (variant,
      // underlying) pair is identical, copy the reference as-is — the surface enum is the same
      ...
      // defensive throw in case that layer is ever loosened. Mirrors Java/Kotlin/TS/Dart.
