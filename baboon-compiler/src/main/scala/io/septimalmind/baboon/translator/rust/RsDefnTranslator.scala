...
object RsDefnTranslator {
  ...
  class RsDefnTranslatorImpl[F[+_, +_]: Applicative2](
    ...
    private def userMapKeyAdapterPath(tpe: TypeRef): Option[String] = mapKeyAdapter.userMapKeyAdapterPath(tpe)

    private def isUserMapKeyEligibleDto(dto: Typedef.Dto): Boolean = mapKeyAdapter.isUserMapKeyEligibleDto(dto)
