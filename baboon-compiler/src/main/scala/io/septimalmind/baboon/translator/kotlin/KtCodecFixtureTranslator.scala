...
trait KtCodecFixtureTranslator {
  ...
  def fixtureTpe(definition: DomainMember.User): Option[TextTree[KtValue]]
}
...
object KtCodecFixtureTranslator {
  final class Impl(
    ...
    private def defnFixtureId(definition: DomainMember): Option[TextTree[KtValue]] = {
      definition match {
    ...
    private def fixtureTpe(id: TypeId): TextTree[KtValue] = {
    ...
    private def doTranslateAdt(adt: Typedef.Adt): TextTree[KtValue] = {
      ...
      q"""object ${fixtureTpe(adt.id)} {
         ...
         |  }
         |
         |  fun randomAll(rnd: $baboonRandom): List<$adtName> {
         |    return listOf(
