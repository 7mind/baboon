...
object RsServiceWiringTranslator {
  class Impl(
    ...
    override def translateServiceRt(domain: Domain): Option[TextTree[RsValue]] = {
      ...
      // In ASYNC mode the `flat_map`/`left_map` combinator bodies generated by
      // the wiring functions inject `impl_.method(..).await` inside the mapping
      // closure (E0728 when the closure is a plain synchronous `FnOnce`). The
      // async trait therefore declares futures-returning closure bounds
      // (`F: FnOnce(..) -> Fut, Fut: Future<Output = ..>`) and `async fn`
      // combinators so `.await` inside the closure is legal. The SYNC path keeps
      // the byte-identical synchronous `FnOnce(B) -> Result<..>` combinators.
    ...
    // In ASYNC mode the `flat_map`/`left_map` combinators are `async fn`s that
    // accept a futures-returning closure (`F: FnOnce(..) -> Fut`); the closure
    // body therefore opens an `async move { .. }` block and the combinator call
    // is awaited. In SYNC mode the closure is a plain `FnOnce` returning the
    // mapped value directly and the combinator is not awaited — keeping the
    ...
    // `Send` to cross `.await`), but the client method returns the historical
    // `Box<dyn std::error::Error>` (matching the codec error type used by the
    // surrounding `?`s). The `?` operator cannot pick a `From` conversion
    // between the two boxed-trait-object error types (E0277:
    // `dyn StdError + Send + Sync: Sized` is not satisfied), so the transport
    // result is mapped explicitly to drop the `+ Send + Sync` marker via an
    // unsizing coercion. SYNC mode's transport already returns
    // `Box<dyn std::error::Error>`, so no map is emitted and the call site stays
    // byte-identical.
    private val transportErrMap: String =
      if (isAsync) ".map_err(|e| -> Box<dyn std::error::Error> { e })" else ""
    ...
    /** Returns the `R` (return-type) used by the underlying invoke function
      ...
      * in async mode it's wrapped in `Pin<Box<dyn Future<Output=…>>>`.
      *
      * The boxed future is intentionally NOT bounded `+ Send`: the generated
      * service trait methods (emitted by RsDefnTranslator as bare `async fn`s,
      * AFIT) produce `?Send` futures, and `BaboonWiringError` itself is not
      * `Send` (it boxes `dyn std::error::Error`). The `JsonMuxer<R>`/`UebaMuxer<R>`
      * runtime contract is generic over `R` with no `Send` bound, so a non-`Send`
      * future is a valid `R`. Requiring `+ Send` here would reject the service
      * futures (E0277 `impl Future … cannot be sent between threads safely`).
      */
    private def wrapperRetType(isJson: Boolean): String = {
      ...
      if (isAsync) s"std::pin::Pin<Box<dyn std::future::Future<Output = $base>>>"
      else base
    }
    ...
    private def generateOneWrapper(spec: WrapperSpec): TextTree[RsValue] = {
      ...
      // In ASYNC mode the `invoke` body clones `impl_`/`rt` into the pinned
      // `async move` block (the future may outlive `&self`), so both generics
