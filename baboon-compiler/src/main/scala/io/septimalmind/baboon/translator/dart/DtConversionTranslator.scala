...
class DtConversionTranslator[F[+_, +_]: Error2](
  ...
  private def transfer(
    ...
    (newTpe, oldTpe) match {
      ...
      // `any` payload is opaque (Uint8List / Object?); we never auto-convert between `any` and a
      ...
      // defensive throw in case that layer is ever loosened. Mirrors Java/Kotlin/TS.
