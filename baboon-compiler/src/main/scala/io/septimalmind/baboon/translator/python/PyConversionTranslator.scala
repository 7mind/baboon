...
final class PyConversionTranslator[F[+_, +_]: Error2](
  ...
  private def transfer(
    ...
    (newTpe, oldTpe) match {
      ...
      // `any` payload is opaque (AnyOpaque ADT); we never auto-convert between `any` and a
      // non-`any` type (validator forbids it). When both old and new are `any` AND their
      // (variant, underlying) pair is identical, copy the reference as-is — the surface ADT is
      // the same `AnyOpaque` and the wire bytes carry their own meta header, independent of the
      // schema version of the containing DTO. Variant or underlying changes are breaking per
      // spec §Evolution and should have been rejected by `BaboonRules.incompatibleAdditions`;
      // we emit a defensive throw in case that layer is ever loosened. Mirrors the pattern in
      // ScConversionTranslator/JvConversionTranslator/DtConversionTranslator/etc.
