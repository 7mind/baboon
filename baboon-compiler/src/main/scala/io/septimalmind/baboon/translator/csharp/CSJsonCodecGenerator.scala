...
class CSJsonCodecGenerator(
      ...
      case TypeRef.Scalar(id) =>
        id match {
          case b: TypeId.BuiltinScalar =>
            CSScalarCodecEmitter.jsonEncode(b, ref, CSScalarCodecEmitter.JsonBooleanFormat.BooleanValue)
          case u: TypeId.User =>
  ...
  private def mkDecoder(tpe: TypeRef, ref: TextTree[CSValue], codecArgs: CodecArguments = CodecArguments.empty): TextTree[CSValue] = {
    def mkReader(bs: TypeId.BuiltinScalar): TextTree[CSValue] = CSScalarCodecEmitter.jsonDecode(bs, ref)
  ...
  private def mkAnyEncoder(a: TypeRef.Any, ref: TextTree[CSValue]): TextTree[CSValue] = {
    ...
    q"$baboonAnyJsonCodec.Encode(ctx, ($csByte)$expectedHex, ${fallback(plan.staticDomain)}, ${fallback(plan.staticVersion)}, ${fallback(plan.staticTypeId)}, $ref)"
  }
  ...
  private def mkAnyDecoder(a: TypeRef.Any, ref: TextTree[CSValue]): TextTree[CSValue] = {
    ...
    q"$baboonAnyJsonCodec.Decode(($csByte)$expectedHex, $ref)"
  }
