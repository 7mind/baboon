...
class CompletionProvider(
      ...
      .map {
        content =>
          ...
          if (position.line < lines.length) {
            ...
            // Alias RHS: `type Name =` (with optional `root` prefix and optional word-or-qualified prefix).
  ...
  private def getPragmaKeyCompletions: Seq[CompletionItem] = {
    (ServiceResultResolver.knownPragmaKeys ++ ServiceContextResolver.knownPragmaKeys).map {
  ...
  private def getTypeCompletions: Seq[CompletionItem] = {
    ...
    workspaceState.getFamily.map {
      family =>
        val types = family.domains.toMap.values.flatMap {
          lineage =>
            lineage.versions.toMap.values.flatMap {
              domain =>
