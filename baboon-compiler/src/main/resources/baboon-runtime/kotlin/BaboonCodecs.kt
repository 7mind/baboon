...
interface BaboonCodecContext {
     ...
     * cross-format conversion (UEBA ↔ JSON) can resolve codecs by `(domain, version, typeid)` from
     * an `AnyMeta` envelope. `null` for the bare `Compact` / `Indexed` singletons; `withFacade(...)`
     * is the single intended construction path for ctxes that thread a facade. See PR 5.1 plumbing
     * (Q6 option (a) — same shape as Scala/C#/Rust).
     */
