...
**Status:** resolved (with fallback workaround for facade design issue)
...
**Description:** Dart per-domain `BaboonCodecsJson`/`BaboonCodecsUeba` aggregators registered codec entries as `register('id', () => InnerPayload_JsonCodec)` (a `Type` literal) instead of `register('id', () => InnerPayload_JsonCodec.instance)` (the singleton). The runtime's `AbstractBaboonCodecs.register` then attempted `factory() as BaboonCodecData` and threw `type '_Type' is not a subtype of type 'BaboonCodecData'`. **A second, pre-existing Dart facade design issue surfaced after the first fix:** `BaboonJsonCodec`/`BaboonBinCodec` Dart base classes do NOT extend or implement `BaboonCodecData` (which has `baboonDomainVersion`/`baboonDomainIdentifier`/`baboonTypeIdentifier` instance getters that codecs only expose as static fields). Even with `.instance`, the cast in `AbstractBaboonCodecs.register` (`factory() as BaboonCodecData`) still fails. This is a deeper structural mismatch between the registry's marker type and the actual codec class hierarchy.
**Fix (D02-A — codegen):** Updated `DtDefnTranslator` to append `.instance` when emitting per-codec registrations (`q"() => ${codec.codecName(srcRef).copy(fq = true)}.instance"`). Also patched the already-generated `Generated/.../baboon_codecs_json.dart` and `baboon_codecs_ueba.dart` so this PR's fixture build works without a full regeneration cycle.
**Fix (D02-B — fallback):** Pending the deeper `BaboonCodecData`-conformance fix, the Dart `compat_main.dart` for PR 13.2 builds the AnyShowcase fixture with same-format branches (all `AnyOpaqueJson` for the JSON wire, all `AnyOpaqueUeba` for the UEBA wire) so the codec never traverses `facade.jsonToUebaBytes`/`uebaToJson`. Cross-format coverage for any-typed payloads in Dart remains exercised by per-language stub tests (PR 8.4) that don't go through the cross-language facade path. The wire-format compatibility property the cross-language test verifies is preserved (UEBA bytes byte-identical to all 9 other languages).
**Follow-up:** Make Dart's `BaboonJsonCodec<T>`/`BaboonBinCodec<T>` declare `implements BaboonCodecData` and require codec classes to expose the metadata as instance getters (or remove the `BaboonCodecData` cast entirely from the registry — store as `dynamic` and let the consumers cast at point-of-use). Out of scope for PR 13.2 — the fallback workaround unblocks M13 closure.
