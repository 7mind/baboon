...
**Status:** open (out-of-scope for round-2 N02 fix)
**Severity:** major
**Location:** `baboon-compiler/src/main/resources/baboon-runtime/python/baboon_codecs_facade.py:443` (`typed = json_codec.decode(BaboonCodecContext.Compact, json.dumps(json_value))`)
**Description:** PR-25.2 unconditionally `json.dumps`-wraps `json_value` before delegating to `json_codec.decode`. When `AnyOpaqueJson.json` is already a JSON-text `str` (the case in `build_json_holder_for_cross_convert` and `test_cross_format_d3_isolated_field_resolves_via_static_fallbacks`, where each branch is constructed with `inner_str = inner_to_json_string(SAMPLE_INNER)`), the call produces a double-encoded payload — pydantic's `model_validate_json` parses the outer layer, sees a string, and rejects with `Input should be an object [type=model_type, input_value='{"x":42}', input_type=str]`.
**Symptom:** `test_cross_format_d3_isolated_field_resolves_via_static_fallbacks` and `test_cross_format_json_holder_encodes_to_ueba_bytewise_canonical` both fail with `DecoderFailure: json_to_ueba_bytes: cannot decode JSON payload of type [my.ok.my.ok/:#Inner] of version '1.0.0'.` Pre-PR-25.2 these tests passed (the facade then forwarded `json_value` unchanged).
**Proposed fix (separate PR):** Either (a) make the facade conditional — `wire = json_value if isinstance(json_value, (str, bytes, bytearray)) else json.dumps(json_value)` — or (b) pin `AnyOpaqueJson.json` to a single representation (always parsed-tree, or always JSON-text) and update both the facade and the test fixtures consistently. Out of scope for the round-2 N02 stub-codec fix per task constraints ("do not touch the Python facade itself or any other test").
