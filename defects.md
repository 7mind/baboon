...
### [FIELD-2026-09-24-D03] Rust `encode_to_json` cannot transcode a UEBA-form `any` payload at all
**Status:** resolved
**Severity:** major
**Location:** `baboon-compiler/src/main/scala/io/septimalmind/baboon/translator/rust/RsBaboonTranslator.scala` (`encode_json_dyn`); `baboon-compiler/src/main/scala/io/septimalmind/baboon/translator/rust/RsJsonCodecGenerator.scala`
**Description:** Follow-up to D01. Threading a context into the Rust facade was necessary but not sufficient: Rust is the only backend whose JSON codec is not generated — `RsJsonCodecGenerator` emitted serde helpers only, and the dyn adapter read `fn encode_json_dyn(&self, _ctx, value)` then called `serde_json::to_value`. `serde::Serialize::serialize(&self, S)` has no room for a context, so `Serialize for AnyOpaque` could reach neither the facade nor the per-field static fallbacks, and errored on the `Ueba` branch. Reproduced against the `any-ok` fixture: `facade.encode_to_json` failed identically with and without a facade in the context. A scoped thread-local carrying the facade was rejected as a fix — `decode_any_field` yields the WIRE meta, which for variants B/C/D is deliberately incomplete, so an ambient facade would have worked for variant A only and failed the realistic decode-UEBA-then-encode-JSON path for the other five, with no kind validation either.
**Fix:** Emitted a context-carrying inherent `encode_json(ctx)` for every generated type, mirroring the UEBA side's `encode_ueba(ctx, writer)`, and pointed the dyn adapter at it. Any-bearing types first rewrite their `any` slots to the JSON branch via the new `any_field_codec::resolve_any_field_for_json` (kind check plus the same static-fallback table `RsUEBACodecGenerator.anyStaticFallbacks` computes), then hand the value to serde unchanged — so renames, hex bytes, decimal-as-number, timestamps, user map-key adapters and ADT wrapping are still produced by the derive and cannot drift. Types with no `any` below them emit a one-line `serde_json::to_value` passthrough. Containment is a least fixpoint over the typespace, iterative because recursive DTOs make the reference graph cyclic. `to_json`/`to_json_value` stay ctx-less and keep refusing UEBA-form payloads. Regression tests: `test/rs-stub/tests/facade_json_any_ctx_tests.rs` — all six DSL variants plus opt/lst/map-value positions transcoding through the facade, the facade-less error, the kind-mismatch check, and a drift guard asserting `encode_json` is byte-identical to `serde_json::to_value` wherever no transcoding happens. Verified with `mdl --seq :ci` (206/206, cross-language acceptance matrix included).
...
### [FIELD-2026-09-23-D02] `BaboonCodecsFacade.IsDeprecated` reports live types as deprecated (walk never advances the type)
**Status:** resolved
**Severity:** major
**Location:** `baboon-compiler/src/main/resources/baboon-runtime/cs/BaboonCodecsFacade.cs:971-1010`
**Description:** For every newer registered version the walk looked for a conversion whose `TypeFrom` is the ORIGINAL value's type; it never moved to the type each step produces. From a 1.0.0 value in a three-version lineage, the 2.0.0 step matches and the 3.0.0 step does not (the 3.0.0 registry holds `v2_0_0.X -> X`), so the method returned `true` while `Convert` succeeded — `ConvertClassified` then reported `Deprecated` for a perfectly convertible value. Two registry shapes were affected: a conversion at every step (DTO and ADT alike), and a conversion that reaches the latest CLASS several versions early, which is what deduplicated codegen emits (`Convert__T1_D2__From__1_0_0` in `testpkg.pkg0` is registered under 2.0.0 yet targets the 3.0.0 class, and 3.0.0 registers nothing for that type — a conversion's `VersionTo()` is the registry's version, not the produced class's, so advancing the version alone does not fix this shape). C#-only: no other backend runtime has `IsDeprecated`.
**Fix:** Rewrote the walk as reachability over the registered conversion graph, following each conversion's `TypeTo`, with a visited set; a type that implements `IBaboonGeneratedLatest`, or one that a conversion of the newest registered version accepts, completes the path. Converters are still never executed. Added the type-keyed `AbstractBaboonConversions.FindConversions(Type)` overload for callers that have no instance of the intermediate types. Regression test: `test/cs-stub/BaboonTests/FacadeDeprecationTests.cs` — real generated `fwde2e.chain` codegen for the every-step shape, plus synthetic registries (whose converters throw, pinning the "does not execute converters" half of the contract) for the reach-latest-early, ADT-member and dead-end shapes.
